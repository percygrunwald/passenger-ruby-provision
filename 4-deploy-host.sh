...
server {
    server_name taiwannights.com $DEPLOY_HOSTNAME;
    listen 80;
    return 301 https://\$server_name\$request_uri;
}

server {
    server_name taiwannights.com;

    listen 443 ssl spdy;
    listen [::]:443 ssl spdy;
    ssl_certificate /etc/letsencrypt/live/taiwannights.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/taiwannights.com/privkey.pem;
    ssl_session_timeout 1d;
    ssl_session_cache shared:SSL:50m;
    ssl_session_tickets off;
    ...
    ## verify chain of trust of OCSP response using Root CA and Intermediate certs
    #ssl_trusted_certificate /path/to/root_CA_cert_plus_intermediates;
    ssl_trusted_certificate /etc/letsencrypt/live/taiwannights.com/chain.pem;
    resolver 8.8.8.8 8.8.4.4 valid=86400;
    resolver_timeout 10;
    return 301 https://www.$server_name$request_uri;
}
EOF
sudo vi /etc/nginx/sites-enabled/$INPUT_USERNAME.conf
echo "Now get certs from ~/letsencrypt/letsencrypt-auto certonly -d $DEPLOY_HOSTNAME -d www.domain.com"
